...
export class JSONWebToken<T extends IPayload> {
  ...
  public static async validateSignatureWithPublicKey({ keyId, jwt, registry }: { keyId: string; jwt: string, registry?: JolocomRegistry }): Promise<boolean> {
    if(!registry) registry = createJolocomRegistry()
